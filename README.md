...
- 一个 Python 环境、约 7 GB，加模型约 10 GB，全部放在项目文件夹里
...
| 纯音乐 | 风格 + 主奏乐器，无人声 | 时长误差 ±12% |
| 参考歌曲 → 换词 | 保留原曲旋律、和声、速度，换成新歌词 | 旋律跟随 85% |
| 参考歌曲 → 换风格 / 换人唱 | 保留原曲旋律和歌词，换曲风、编曲或演唱者（歌词可直接粘 SRT/LRC 字幕） | 旋律跟随 92% |
...
**需要：** Windows 10/11 64 位、NVIDIA 显卡和 570 以上的驱动、约 25 GB 磁盘空间、能访问国内镜像的网络。
...
所有东西都装在项目文件夹里（`.venv\`、`tools\`、`models\`），卸载就是删掉整个文件夹。
...
- **长度**：YuE2 没有时长参数。纯音乐按段落裁到目标长度（误差 ±12%）；带歌词的歌长度由歌词决定，但波动很大
  （同样 6 句歌词，实测 74~207 秒，模型有时会自己加很长的器乐段）；哼唱和参考歌曲跟随原音频长度。
